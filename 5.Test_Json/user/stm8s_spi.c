...
void SPI_Init(SPI_FirstBit_TypeDef FirstBit, SPI_BaudRatePrescaler_TypeDef BaudRatePrescaler, SPI_Mode_TypeDef Mode, SPI_ClockPolarity_TypeDef ClockPolarity, SPI_ClockPhase_TypeDef ClockPhase, SPI_DataDirection_TypeDef Data_Direction, SPI_NSS_TypeDef Slave_Management, uint8_t CRCPolynomial)
{
  ...
  //(IS_SPI_FIRSTBIT_OK(FirstBit));
  //(IS_SPI_BAUDRATE_PRESCALER_OK(BaudRatePrescaler));
  //(IS_SPI_MODE_OK(Mode));
  //(IS_SPI_POLARITY_OK(ClockPolarity));
  //(IS_SPI_PHASE_OK(ClockPhase));
  //(IS_SPI_DATA_DIRECTION_OK(Data_Direction));
  //(IS_SPI_SLAVEMANAGEMENT_OK(Slave_Management));
  //(IS_SPI_CRC_POLYNOMIAL_OK(CRCPolynomial));
...
void SPI_Cmd(FunctionalState NewState)
{
  ...
  //(IS_FUNCTIONALSTATE_OK(NewState));
...
{
  /* Check function parameters */
  //(IS_SPI_DIRECTION_OK(SPI_Direction));
...
{
  //(IS_SPI_CLEAR_FLAGS_OK(SPI_FLAG));
  /* Clear the flag bit */
