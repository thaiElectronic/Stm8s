...
char *Uart_int_to_char(unsigned int vrui_data)
{
	 char vruc_arr[10];
	int vruc_temp = vrui_data, i=0, vruc_data, j=0;
	
	while(vruc_temp)
	{
		vruc_arr[i] = (vruc_temp % 10)+'0';
		vruc_temp /= 10;
		i++;
	}
	vruc_arr[i] = 0x00;
	...
	for(j=0; j < i/2; j++)
	{
		vruc_data = vruc_arr[j];
		vruc_arr[j] = vruc_arr[i-1-j];
		vruc_arr[i-1-j] = vruc_data;
	}
	if(vrui_data==0)
		return "0";
	return vruc_arr;
